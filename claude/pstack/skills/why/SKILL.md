...
Claude Code port of pstack's `why`. The templates, epistemics framework, and source playbooks in `references/` are upstream's.
...
- `review-heavy`: spawn one `Explore` agent per available category with the Agent tool, all in one message.
...
List the MCP servers and tools available in this Claude Code session, including deferred MCP tools named in system reminders. Map each to one evidence category:
...
In `review-heavy`, each investigator is an `Explore` agent that owns exactly one category. Otherwise, you run the categories yourself and record each one's findings separately before moving on.
