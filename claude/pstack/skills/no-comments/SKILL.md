...
Claude Code port of pstack's `no-comments`. Comment Sicko's rules are upstream's, in `references/comment-sicko.md`.
...
- `review-heavy`: spawn `pstack:comment-sicko` with the Agent tool. Its prompt gives the absolute path of this skill's `references/comment-sicko.md` (this skill's base directory plus that path) and the diff text. Do not restate its rules. It reports only, and you apply the deletions you accept.
